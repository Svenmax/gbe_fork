#!/usr/bin/env python3
"""Phase 2.7 Step 1: Coupling analysis for extracting inventory/item functions.

Targets 20 Steam_Game_Coordinator:: member functions in steam_game_coordinator.cpp
that are candidates to move to a new TU (gbe_dota_inventory_coordinator.cpp):

  item_id_local_to_network / item_id_network_to_local
  item_to_gcstruct / item_to_gcprotobuf
  handle_set_item_pos / handle_delete_item
  handle_set_item_style / handle_adjust_equip_state / handle_set_multiple_item_pos
  callback_items_received / callback_items_removed
  callback_item_updated / callback_item_deleted
  load_items_from_file / save_items_to_file
  set_item_pos / delete_item
  request_user_items / find_items_request / remove_user_items

These are the inventory/item management functions. They are non-contiguous
(interleaved with handle_motd_request, handle_respawn, callback_respawn_request
which stay, plus welcome/init/infrastructure functions in the middle of the
range), so the extract script must locate each span by name + brace tracking.

Functions explicitly LEFT IN main file (Phase 2.8 candidates or non-item):
  - handle_motd_request, handle_respawn (GC general handlers, not item)
  - callback_respawn_request (respawn, not item)
  - callback_client_welcome, callback_server_welcome,
    GBE_MaybePrimeDotaServerWelcomeFromCache, GBE_PushDotaLoginSyncMessages
    (welcome/login — Phase 2.8)
  - on_client_connected, on_client_disconnected (connection lifecycle, not
    pure item; they call lobby reset code that stays in main file)
  - steam_network_callback, steam_run_every_runcb, initialize_gc,
    shutdown_gc, on_appid_changed (core infrastructure, stays)
...
Output: tools/_phase27_coupling_report.txt
"""
import os
import re
import sys
...
REPORT_PATH = "/workspace/tools/_phase27_coupling_report.txt"

# 20 target member function names. No overloads (verified via grep).
TARGET_FUNCTIONS = [
...
# Phase 2.6 added NO new externalized symbols (List Y was 0). These have
# external linkage and live in steam_game_coordinator.cpp with extern
...
def main():
    ...
    out("=" * 80)
    out("PHASE 2.7: INVENTORY/ITEM FUNCTIONS COUPLING ANALYSIS REPORT")
    out(f"File: {CPP_PATH} ({len(lines)} lines)")
    out("Purpose: Support refactoring — extracting 20 inventory/item member")
    out("         functions to a new TU (gbe_dota_inventory_coordinator.cpp)")
    out("=" * 80)
    out("")
    out("METHODOLOGY")
    out("-----------")
    out("- 20 target member function names were located by matching")
    ...
    out("=" * 80)
    if list_y:
        out(f"1. PHASE 2.7-step2 REQUIRED: Externalize {len(list_y)} List Y static symbols.")
    else:
        out("1. PHASE 2.7-step2 SKIPPABLE: No List Y symbols.")
    out("")
    if list_x:
        out(f"2. PHASE 2.7-step3: Move {len(list_x)} List X static symbols + {len(target_ranges)} target")
        out("   definitions to gbe_dota_inventory_coordinator.cpp.")
    else:
        out(f"2. PHASE 2.7-step3: No List X static symbols — only {len(target_ranges)} target definitions move.")
    out("")
