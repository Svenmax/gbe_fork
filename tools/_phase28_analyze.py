#!/usr/bin/env python3
"""Phase 2.8 Step 1: Coupling analysis for extracting welcome/hello functions.

Targets 5 Steam_Game_Coordinator:: member functions in steam_game_coordinator.cpp
that are candidates to move to a new TU (gbe_dota_welcome_coordinator.cpp):

  GBE_PatchDotaLoginCacheSubscribedInventory  (patches login CacheSubscribed inventory)
  callback_client_welcome                     (builds ClientWelcome)
  callback_server_welcome                     (builds ServerWelcome)
  GBE_MaybePrimeDotaServerWelcomeFromCache    (primes cached ServerWelcome)
  GBE_PushDotaLoginSyncMessages               (queues login CacheSubscribed replay)

These are the welcome/hello/login-sync functions. They are non-contiguous
(interleaved with handle_motd_request, handle_respawn, callback_respawn_request,
steam_network_callback, initialize_gc, on_client_* which stay), so the extract
script must locate each span by name + brace tracking.

Functions explicitly LEFT IN main file (core infrastructure or non-welcome):
  - handle_motd_request, handle_respawn (GC general handlers)
  - callback_respawn_request (respawn, not welcome)
  - build_protomsg_header, push_incoming, push_incoming_now (core msg infra)
  - initialize_gc, shutdown_gc, on_appid_changed (lifecycle)
  - on_client_connected, on_client_disconnected (connection lifecycle)
  - steam_network_callback, steam_run_every_runcb (callbacks)
  - handle_dota_client_message, SendMessage_, RetrieveMessage (core dispatch)
...
Output: tools/_phase28_coupling_report.txt
"""
import os
import re
import sys
...
REPORT_PATH = "/workspace/tools/_phase28_coupling_report.txt"

# 5 target member function names. No overloads (verified via grep).
TARGET_FUNCTIONS = [
...
# Symbols already externalized in earlier phases (Phase 2.2 / 2.3a / 2.4a / 2.5).
# Phase 2.6 added NO new externalized symbols (List Y was 0).
# Phase 2.7 added NO new extern symbols (deser_var moved to header as inline template,
# not extern; GCMsgHdrEx_t moved to header as struct def; parse_protomsg got explicit
# instantiations but stays defined in main .cpp).
# These have external linkage and live in steam_game_coordinator.cpp with extern
# declarations in gbe_dota_gc_internal.h.
ALREADY_EXTERNAL = {
    ...
    # Phase 2.5 — lobby-snapshot/build shared symbols (List Y + List X->Y)
...
def main():
    ...
    # --- 1. Locate all target function definitions ---
    target_ranges = []
    ...
    for s, e, fname in target_ranges:
        print(f"  {fname:45s}  lines {s+1:5d}-{e+1:5d}  ({e-s+1} lines)")
    ...
    # --- 5. Find references from target functions to other member functions ---
    member_refs = {}
    ...
    out("=" * 80)
    out("PHASE 2.8: WELCOME/HELLO FUNCTIONS COUPLING ANALYSIS REPORT")
    out(f"File: {CPP_PATH} ({len(lines)} lines)")
    out("Purpose: Support refactoring — extracting 5 welcome/hello/login-sync")
    out("         member functions to a new TU (gbe_dota_welcome_coordinator.cpp)")
    out("=" * 80)
    out("")
    out("METHODOLOGY")
    out("-----------")
    out("- 5 target member function names were located by matching")
    out("  `^.*Steam_Game_Coordinator::FUNC_NAME(` at column 0.")
    out("- Function end was determined by brace-depth tracking.")
    out("- All file-scope `static` symbols were collected & deduplicated by name.")
    out("- Each static symbol's references were classified 'inside target' or 'elsewhere'.")
    out("")
    out("TARGET FUNCTIONS (incl. overloads)")
    out("----------------------------------")
    for s, e, fname in target_ranges:
        out(f"  {fname:45s}  lines {s+1:5d}-{e+1:5d}  ({e-s+1} lines)")
    total_target_lines = sum(e - s + 1 for s, e, _ in target_ranges)
    out(f"  {'TOTAL':45s}  {total_target_lines} lines across {len(target_ranges)} definitions")
    out("")
    ...
    out("file-scope `static` there. (Verify no transitive List Z dependency!)")
    out("")
    ...
    out("NOTE: If a List Y function definition has a default argument, the")
    out("`= value` must be stripped from the DEFINITION (header keeps it),")
    out("else MSVC error C2572 (redefinition of default argument).")
    out("")
    if list_y:
    ...
    out("=" * 80)
    if list_y:
        out(f"1. PHASE 2.8-step2 REQUIRED: Externalize {len(list_y)} List Y static symbols.")
    else:
        out("1. PHASE 2.8-step2 SKIPPABLE: No List Y symbols.")
    out("")
    if list_x:
        out(f"2. PHASE 2.8-step3: Move {len(list_x)} List X static symbols + {len(target_ranges)} target")
        out("   definitions to gbe_dota_welcome_coordinator.cpp.")
    else:
        out(f"2. PHASE 2.8-step3: No List X static symbols — only {len(target_ranges)} target definitions move.")
    out("")
    out("3. PREMAKE: No change needed — `dll/**` glob auto-collects sources.")
    out("")
    out("=" * 80)
    out("END OF REPORT")
    out("=" * 80)
