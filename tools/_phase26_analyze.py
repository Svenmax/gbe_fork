#!/usr/bin/env python3
"""Phase 2.6 Step 1: Coupling analysis for extracting lobby launch/teardown flow functions.

Targets 13 Steam_Game_Coordinator:: member functions in steam_game_coordinator.cpp
that are candidates to move to a new TU (gbe_dota_lobby_launch_coordinator.cpp):
...
These functions form the lobby launch / state-advance / post-game teardown
flow. They are non-contiguous (interleaved with GBE_GetDotaJoinableCustomLobbiesHTTPJSON,
ResetGCMemory, GBE_GetDotaLobbyOwnerName which stay in the main file), so the
extract script must locate each span by name + brace tracking.
...
It also lists symbols already externalized (per Phase 2.2 / 2.3a / 2.4a / 2.5)
that are referenced from the target functions, to verify the prep work is
sufficient.

Output: tools/_phase26_coupling_report.txt
"""
import os
import re
import sys
...
REPORT_PATH = "/workspace/tools/_phase26_coupling_report.txt"

# 13 member function names. No overloads (verified via grep).
TARGET_FUNCTIONS = [
    "GBE_TryQueueDotaPrelaunch021",
    "GBE_SetDotaLobbyMemberConnected",
    "GBE_SetDotaLobbyMemberRuntimeState",
...
def main():
    ...
    out("=" * 80)
    out("PHASE 2.6: LOBBY LAUNCH/TEARDOWN FLOW COUPLING ANALYSIS REPORT")
    out(f"File: {CPP_PATH} ({len(lines)} lines)")
    out("Purpose: Support refactoring — extracting 13 member functions to a new TU")
    out("         (gbe_dota_lobby_launch_coordinator.cpp)")
    out("=" * 80)
    out("")
    out("METHODOLOGY")
    out("-----------")
    out("- 13 target member function names were located by matching")
    ...
    out("  ({0} unique names, {1} with overloads)".format(
    ...
    out("=" * 80)
    out("These symbols were promoted to external linkage in Phase 2.2 / 2.3a / 2.4a / 2.5.")
    ...
    out("=" * 80)
    if list_y:
        out(f"1. PHASE 2.6-step2 REQUIRED: Externalize {len(list_y)} List Y static")
        out("   symbols. Write tools/_phase26_externalize.py to:")
        ...
        out("1. PHASE 2.6-step2 SKIPPABLE: No List Y symbols — earlier phases'")
        out("   externalization work already covers all cross-TU sharing needs.")
    out("")
    if list_x:
        out(f"2. PHASE 2.6-step3: Move {len(list_x)} List X static symbols + {len(target_ranges)} target")
        out("   definitions to gbe_dota_lobby_launch_coordinator.cpp. List X symbols")
        out("   keep their `static` keyword (internal to the new TU).")
    else:
        out(f"2. PHASE 2.6-step3: No List X static symbols to move — only the {len(target_ranges)}")
