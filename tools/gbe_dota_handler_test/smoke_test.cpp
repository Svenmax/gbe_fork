...
static void test_inventory_equip_full_forward()
{
    ...
    TEST_ASSERT(tf.recorder.actions[11].reason == "7034_owner_hero_known_client", "12: early replay should reuse the owner hero client seam");
...
static void test_match_7034_host_showcase_repush_guard_marks_once()
{
    ...
    TEST_ASSERT_EQ(tf.recorder.actions.size(), 8u, "first showcase request should publish hero, replay SO state, dual-GC wearable refresh, then respond");
