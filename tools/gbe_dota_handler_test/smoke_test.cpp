...
static void test_inventory_equip_full_forward()
{
    ...
    TEST_ASSERT_EQ(tf.recorder.actions.size(), 17u, "should publish inferred hero to both GCs before forward actions and early replay");
    ...
    TEST_ASSERT_EQ(tf.recorder.actions[11].type, GBE_DotaActionType::ServerGcForward, "12: early owner hero client equipped cache replace");
    TEST_ASSERT_EQ(action_emsg(tf.recorder.actions[11]), 0u, "12: client cache replace emsg=0");
    TEST_ASSERT(tf.recorder.actions[11].server_gc_unsubscribe_first, "12: client cache replace should unsubscribe first");
    TEST_ASSERT(tf.recorder.actions[11].reason == "7034_owner_hero_known_client_cache", "12: client cache reason should identify owner-known replace");
    TEST_ASSERT_EQ(action_emsg(tf.recorder.actions[12]), 26u, "13: early owner hero client update");
    TEST_ASSERT(tf.recorder.actions[12].reason == "7034_owner_hero_known_client", "13: early replay should reuse the owner hero client seam");
    TEST_ASSERT_EQ(action_emsg(tf.recorder.actions[13]), 1029u, "14: first server wearable refresh");
    TEST_ASSERT(tf.recorder.actions[13].server_gc, "14: first wearable refresh should target server GC");
    TEST_ASSERT_EQ(action_emsg(tf.recorder.actions[14]), 1029u, "15: delayed server wearable refresh");
    TEST_ASSERT(tf.recorder.actions[14].server_gc, "15: delayed wearable refresh should target server GC");
    TEST_ASSERT_EQ(action_emsg(tf.recorder.actions[15]), 1029u, "16: first client wearable refresh");
    TEST_ASSERT(!tf.recorder.actions[15].server_gc, "16: first client wearable refresh should target client GC");
    TEST_ASSERT_EQ(action_emsg(tf.recorder.actions[16]), 1029u, "17: delayed client wearable refresh");
    TEST_ASSERT(!tf.recorder.actions[16].server_gc, "17: delayed client wearable refresh should target client GC");
...
static void test_production_dispatcher_registry_contract()
{
    ...
    TEST_ASSERT_EQ(view.size, 52u, "production registry should retain all canonical entries");
...
static void test_match_7034_connected_player_updates_runtime_before_response()
{
    ...
    TEST_ASSERT_EQ(tf.recorder.actions.size(), 10u, "7034 should publish runtime state, replay host cache, replace client equipped cache, update client items, dual-GC wearable refresh, queue runtime update, then respond");
    ...
    TEST_ASSERT_EQ(tf.recorder.actions[2].type, GBE_DotaActionType::ServerGcForward, "third action should replace client equipped cache");
    TEST_ASSERT_EQ(tf.recorder.actions[2].msg_type, 0u, "third action should be equipped CacheSubscribed replace");
    TEST_ASSERT(tf.recorder.actions[2].server_gc_unsubscribe_first, "client cache replace should unsubscribe first");
    TEST_ASSERT(tf.recorder.actions[2].reason == "7034_owner_hero_known_client_cache", "client cache reason should identify owner hero transition");
    TEST_ASSERT_EQ(tf.recorder.actions[3].msg_type, 26u, "fourth action should push current hero equipped cache update to client GC");
    TEST_ASSERT(tf.recorder.actions[3].reason == "7034_owner_hero_known_client", "client item update reason should identify owner hero transition");
    TEST_ASSERT_EQ(tf.recorder.actions[4].msg_type, 1029u, "fifth action should request server wearable refresh");
    TEST_ASSERT_EQ(tf.recorder.actions[4].steam_id, owner_steam_id, "wearable refresh should target the lobby owner");
    TEST_ASSERT_EQ(tf.recorder.actions[4].delay, 0.1, "first wearable refresh should use the default hot-cache delay");
    TEST_ASSERT(tf.recorder.actions[4].server_gc, "first wearable refresh should be delivered by the server GC");
    TEST_ASSERT_EQ(tf.recorder.actions[5].msg_type, 1029u, "sixth action should request delayed server wearable refresh");
    TEST_ASSERT_EQ(tf.recorder.actions[5].steam_id, owner_steam_id, "delayed wearable refresh should target the lobby owner");
    TEST_ASSERT_EQ(tf.recorder.actions[5].delay, 1.5, "second wearable refresh should cover cold-start entity initialization");
    TEST_ASSERT(tf.recorder.actions[5].server_gc, "delayed wearable refresh should be delivered by the server GC");
    TEST_ASSERT_EQ(tf.recorder.actions[6].msg_type, 1029u, "seventh action should request client wearable refresh");
    TEST_ASSERT_EQ(tf.recorder.actions[6].delay, 0.1, "client wearable refresh should use the hot-cache delay");
    TEST_ASSERT(!tf.recorder.actions[6].server_gc, "client wearable refresh should be delivered by the client GC");
    TEST_ASSERT_EQ(tf.recorder.actions[7].msg_type, 1029u, "eighth action should request delayed client wearable refresh");
    TEST_ASSERT_EQ(tf.recorder.actions[7].delay, 1.5, "delayed client wearable refresh should cover cold-start entity initialization");
    TEST_ASSERT(!tf.recorder.actions[7].server_gc, "delayed client wearable refresh should be delivered by the client GC");
    expect_push_action(tf.recorder.actions[8], 26u, "ninth action should queue runtime update");
    expect_push_payload(tf.recorder.actions[9], 7034u, "tenth action should push 7034 response with payload");
...
static void test_match_7034_host_showcase_repush_guard_marks_once()
{
    ...
    TEST_ASSERT_EQ(tf.recorder.actions.size(), 9u, "first showcase request should publish hero, replace client equipped cache, replay SO state, dual-GC wearable refresh, then respond");
    ...
    TEST_ASSERT_EQ(tf.recorder.actions[2].type, GBE_DotaActionType::ServerGcForward, "third showcase action should replace client equipped cache");
    TEST_ASSERT_EQ(tf.recorder.actions[2].msg_type, 0u, "third showcase action should be equipped CacheSubscribed replace");
    TEST_ASSERT(tf.recorder.actions[2].server_gc_unsubscribe_first, "client cache replace should unsubscribe first");
    TEST_ASSERT(tf.recorder.actions[2].reason == "7034_owner_hero_known_client_cache", "owner-known client cache reason should be preserved");
    TEST_ASSERT_EQ(tf.recorder.actions[3].msg_type, 26u, "fourth showcase action should replay hero item SO updates to the client GC");
    TEST_ASSERT(tf.recorder.actions[3].reason == "7034_owner_hero_known_client", "owner-known client replay reason should be preserved");
    TEST_ASSERT_EQ(tf.recorder.actions[4].msg_type, 1029u, "fifth showcase action should request the first server wearable refresh");
    TEST_ASSERT(tf.recorder.actions[4].server_gc, "fifth showcase action should target server GC");
    TEST_ASSERT_EQ(tf.recorder.actions[5].msg_type, 1029u, "sixth showcase action should request the delayed server wearable refresh");
    TEST_ASSERT(tf.recorder.actions[5].server_gc, "sixth showcase action should target server GC");
    TEST_ASSERT_EQ(tf.recorder.actions[6].msg_type, 1029u, "seventh showcase action should request the first client wearable refresh");
    TEST_ASSERT(!tf.recorder.actions[6].server_gc, "seventh showcase action should target client GC");
    TEST_ASSERT_EQ(tf.recorder.actions[7].msg_type, 1029u, "eighth showcase action should request the delayed client wearable refresh");
    TEST_ASSERT(!tf.recorder.actions[7].server_gc, "eighth showcase action should target client GC");
    expect_push_payload(tf.recorder.actions[8], 7034u, "ninth action should push 7034 response with payload");
    const std::string first_showcase_summary = dota7034_response_summary(tf.recorder.actions[8]);
    TEST_ASSERT(
...
static void test_match_7034_host_showcase_replays_wearables_after_strategy_mark()
{
    ...
    TEST_ASSERT_EQ(
        ...
        8u,
        "PRE_GAME after strategy mark should replay server cache, client equipped replace, client SO, dual-GC 1029, then respond");
    ...
    TEST_ASSERT_EQ(tf.recorder.actions[1].type, GBE_DotaActionType::ServerGcForward, "second action should replace client equipped cache");
    TEST_ASSERT_EQ(tf.recorder.actions[1].msg_type, 0u, "second action should be equipped CacheSubscribed replace");
    TEST_ASSERT(tf.recorder.actions[1].server_gc_unsubscribe_first, "client cache replace should unsubscribe first");
    TEST_ASSERT(tf.recorder.actions[1].reason == "7034_owner_hero_known_client_cache", "client cache reason should identify owner-known replace");
    TEST_ASSERT_EQ(tf.recorder.actions[2].msg_type, 26u, "third action should push hero equipped SO updates to client GC");
    TEST_ASSERT(tf.recorder.actions[2].reason == "7034_owner_hero_known_client", "client SO reason should identify owner-known replay");
    TEST_ASSERT_EQ(tf.recorder.actions[3].msg_type, 1029u, "fourth action should request first server wearable refresh");
    TEST_ASSERT_EQ(tf.recorder.actions[3].delay, 0.1, "first server wearable refresh should use hot-cache delay");
    ...
    TEST_ASSERT_EQ(tf.recorder.actions[4].delay, 1.5, "second server wearable refresh should cover cold-start spawn");
    TEST_ASSERT(tf.recorder.actions[4].server_gc, "fifth action should target server GC");
    TEST_ASSERT_EQ(tf.recorder.actions[5].msg_type, 1029u, "sixth action should request first client wearable refresh");
    TEST_ASSERT_EQ(tf.recorder.actions[5].delay, 0.1, "first client wearable refresh should use hot-cache delay");
    ...
    TEST_ASSERT_EQ(tf.recorder.actions[6].delay, 1.5, "second client wearable refresh should cover cold-start spawn");
    TEST_ASSERT(!tf.recorder.actions[6].server_gc, "seventh action should target client GC");
    expect_push_payload(tf.recorder.actions[7], 7034u, "eighth action should push 7034 response");
...
static void test_match_7034_host_pre_game_equip_not_consumed_by_team_showcase()
{
    ...
    TEST_ASSERT_EQ(
        tf.recorder.actions.size(),
        8u,
        "PRE_GAME after TEAM_SHOWCASE should replay server cache, client equipped replace, client SO, dual-GC 1029, then respond");
    ...
    TEST_ASSERT_EQ(tf.recorder.actions[1].type, GBE_DotaActionType::ServerGcForward, "PRE_GAME second action should replace client equipped cache");
    TEST_ASSERT_EQ(tf.recorder.actions[1].msg_type, 0u, "PRE_GAME client cache replace should be equipped CacheSubscribed");
    TEST_ASSERT(tf.recorder.actions[1].server_gc_unsubscribe_first, "PRE_GAME client cache replace should unsubscribe first");
    TEST_ASSERT(tf.recorder.actions[1].reason == "7034_owner_hero_known_client_cache", "PRE_GAME client cache reason should identify owner-known replace");
    TEST_ASSERT_EQ(tf.recorder.actions[2].msg_type, 26u, "PRE_GAME should push hero equipped SO updates to client GC");
    TEST_ASSERT_EQ(tf.recorder.actions[3].msg_type, 1029u, "PRE_GAME should request first server wearable refresh");
    TEST_ASSERT(tf.recorder.actions[3].server_gc, "first PRE_GAME 1029 should target server GC");
    TEST_ASSERT_EQ(tf.recorder.actions[5].msg_type, 1029u, "PRE_GAME should request first client wearable refresh");
    TEST_ASSERT(!tf.recorder.actions[5].server_gc, "first client PRE_GAME 1029 should target client GC");
    expect_push_payload(tf.recorder.actions[7], 7034u, "PRE_GAME should push 7034 response");
