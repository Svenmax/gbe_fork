...
过渡态：post-login **registry 主干**（**52** 条）+ **Hello 显式 welcome handler** + **template 白名单**（TEMPLATE_ONLY / REGISTRY_DEFENSIVE）+ **少量条件/观察 fallback**（8744、late steam chain）+ wrapped dead 7047 fallback。
...
1. 路由仍多轨（registry / 条件 fallback / 白名单 template / Hello welcome 轨分离）。
