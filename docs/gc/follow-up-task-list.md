...
- [x] Push / response seam phase 4.
  - Goal: extend the practice-lobby response helper to one more covered call site without crossing into a new handler family.
...
- [x] Launch-state grouped payload build seam.
  - Goal: remove direct request indexing from the production launch-state push path while preserving existing payload build failure handling.
...
- [x] Launch-state capture availability mapping seam.
  ...
  - Stop condition: no capture source, captured lobby field mapping, target local steam id read, last-pushed state read, payload build, response push, or debug-log behavior changed.
