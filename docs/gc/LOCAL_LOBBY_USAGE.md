...
- **members / chat / broadcast**：slot、chat、member_coordinator；7009 join 与 7272 leave 的 chat channel 字段组经 `apply_chat_channel()` / `clear_chat_channel()` 写入 Local，并记录当前 generation 作为 shared adopt 来源判定，host sync、7010/7014 push 与 postgame tombstone 顺序仍由 chat handler 管理；7149 join、7367 update 与 8054 close 的 broadcast channel 字段组经 `apply_broadcast_channel()` / `patch_broadcast_channel()` / `clear_broadcast_channel()` 写入 Local，并记录当前 generation 作为 shared adopt 来源判定，publish、details update 与 ack 顺序仍由 chat handler 管理。同 generation shared snapshot adopt 优先保留 Local chat/broadcast 字段组。
- **kick member snapshot apply**：7081 generic kick 成功后的 Local 快照写回经 `apply_lobby_member_kick_snapshot()`，并记录当前 generation 作为 members restore 来源判定；generic kick 调用、shared publish、details update 与日志顺序仍由 slot handler 管理。
...
- **server_id / connect / match_id / game_start_time / room_name restore/apply**：shared-to-local restore 由 source-aware plan 一次应用；空 connect、零 match_id、零 server_id 与零 start_time 保留 Local 值，room_name 继续允许 shared 清空。4508 runtime connect 的 Local 写入经 `apply_runtime_connect()`，空 connect 与相同 connect 保留既有字段，并记录当前 generation 作为 shared restore 来源判定，LAN preserve guard 与 shared Store compare_update 仍由 post-login handler 管理；generic metadata publish 的 connect/server_id Local 写入经 `apply_runtime_metadata()`，并记录当前 generation 作为 shared restore 来源判定，shared Store compare_update、generic metadata publish 与日志顺序仍由 publish coordinator 管理；recover coordinator 的 derived server_id 写入经 `apply_lobby_server_id()`，并记录当前 generation 作为 shared restore 来源判定，derived guard、shared Store compare_update 与 diagnostics 顺序仍由 recover coordinator 管理。同 generation runtime metadata local apply 优先保留 Local connect/server_id 字段组。
- **owner_connected / owner_team / owner_slot restore**：shared-to-local restore 经 `restore_lobby_owner_runtime_from_shared()` 聚合 owner runtime 字段组，并在同 generation owner runtime local apply 后保留 Local 字段组；单字段 restore helper 仍保留匹配值/变更值语义。launch member connection owner 分支与 connection lifecycle owner connect/disconnect 分支经 `apply_lobby_owner_connected()` 写入 Local 并保持 changed 聚合、suppress guard 与 postgame publish suppression 语义；7034 draft owner team/slot 与 7047 set team slot owner 分支经 `apply_lobby_owner_team()` 与 `apply_lobby_owner_slot()` 写入 Local，draft owner slot 零值 guard、7047 member update、bot difficulty 与 publish 顺序仍在 handler。
- **owner_name local apply**：generic lobby owner adoption 的 owner_name 写入经 `apply_lobby_owner_name()`，并记录当前 generation 作为 shared adopt 来源判定；同 generation shared snapshot adopt 优先保留 Local owner_name，owner adoption 决策、local owner publish、metadata publish 与日志顺序仍由 member coordinator 管理。
...
- **members restore**：shared-to-local runtime restore 的 members 字段组经 `restore_lobby_members()` 写入 Local；同 generation 本地 member kick snapshot 优先保留 Local members，changed 聚合、sync settings、rich presence replay 与 login sync 顺序仍由 restore coordinator 管理。
- **options restore**：shared-to-local restore 的 game_mode/server_region/lan/ping/allow_cheats/fill_with_bots/allow_spectating/pass_key/visibility/bot_* 经 `compose_shared_lobby_options_restore_plan()` 与 `apply_shared_lobby_options_restore_plan()` 返回字段组变更；同 generation generic options capture 优先保留 Local 字段组。
- **details update local apply**：7046 set details 的 room/options/custom_game 字段组经 `apply_lobby_details_update()` 写入 Local，并按 runtime/options/custom_game 三个字段组记录当前 generation 作为 shared restore 来源判定；同 generation shared runtime/options/custom_game restore 优先保留 Local details update 结果，lobby id mismatch 日志、custom game normalize、arcade slot normalize、details push 与 diagnostics 顺序仍由 create handler 管理。
- **cache restore/adopt**：shared-to-local restore 的 has_cache_version/cache_version/has_cache_service_id/cache_service_id/cache_service_list/has_cache_sync_version/cache_sync_version 经 `compose_shared_lobby_cache_restore_plan()` 与 `apply_shared_lobby_cache_restore_plan()` 返回字段组变更；full shared snapshot adopt 对同一 cache 字段组执行相同 generation guard；同 generation cache subscription metadata apply 优先保留 Local 字段组。
- **cache subscription metadata local apply**：`GBE_RecordDotaLobbyCacheSubscriptionState(...)` 解析后的 cache metadata 字段组经 `apply_cache_subscription_metadata()` 写入 Local，并记录当前 generation 作为 shared restore 来源判定；owner SOID guard、日志、summary 与 publish 顺序仍由 publish coordinator 管理。
- **custom_game restore**：shared-to-local restore 经 `restore_lobby_custom_game_from_shared()` 返回变更；比较复用 `custom_game_details_equal()`，同 generation generic custom_game capture 优先保留 Local 字段组。
- **custom game loading metadata local apply**：8052 started loading 的 custom_game_id / start_time 经 `apply_custom_game_loading_metadata()` 写入 Local，并记录当前 generation 作为 shared restore 来源判定；同 generation shared custom_game 与 game_start_time restore 保留 Local 字段。零值输入保留既有字段，launch setup 计算与 lifecycle decision 顺序仍由 lifecycle handler 管理。
- **bot difficulty team local apply**：7047 set team slot 的 radiant/dire bot difficulty 经 `apply_lobby_bot_difficulty_for_team()` 写入 Local，并记录当前 generation 作为 shared options restore 来源判定；同 generation shared options restore 优先保留 Local bot difficulty 字段组，bot team 推导与 request guard 仍由 slot handler 管理。
...
- **launch_4511_seen**：仅匹配 lobby 的 4511 通知通过 `mark_launch_4511_seen()` 幂等标记；首次变更才 publish shared state，并记录当前 generation 作为 shared restore 来源判定。
- **launch_4511_seen restore**：shared-to-local restore 通过 `restore_launch_4511_seen()` 返回字段变更；同 generation 本地 4511 标记优先保留，Coordinator 继续聚合 restore 的 `changed` 结果。
