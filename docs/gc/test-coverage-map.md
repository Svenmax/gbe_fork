...
- Launch-state push planner, payload build request, grouped payload build, target/shared/capture/captured mapping, and action-sequence seams are covered by `gbe_dota_lobby_flow_test`; full launch coordinator harness coverage remains deferred around restore, capture source, settings reads, logging, and coordinator selection.
