...
| 2026-07-26 | direct Local lobby parenthesized field write guard：audit 10d 覆盖 `(GBE_local_lobby).state = 1u` parenthesized direct field write 旁路，生产代码保持不变 | `.monkeycode/specs/gc-direct-local-lobby-parenthesized-field-write-guard/`、`tools/_audit_gc_refactor.py` |
| 2026-07-26 | direct target Local lobby indexed mutating method guard：补充 audit 10d 回归，显式覆盖 `options.client_target->GBE_local_lobby.members[0].slots.clear()` target indexed member mutating method 旁路，生产代码与 audit 实现保持不变 | `.monkeycode/specs/gc-direct-target-local-lobby-indexed-mutating-method-guard/`、`tools/test_audit_gc_refactor.py` |
| 2026-07-26 | direct Local lobby indexed mutating method guard：audit 10d 覆盖 `GBE_local_lobby.members[0].slots.clear()` 这类 indexed member mutating method 旁路，生产代码保持不变 | `.monkeycode/specs/gc-direct-local-lobby-indexed-mutating-method-guard/`、`tools/_audit_gc_refactor.py` |
| 2026-07-26 | direct Local lobby indexed compound write guard：补充 audit 10d 回归，显式覆盖 `GBE_local_lobby.members[0].team += 1u` indexed compound mutation，生产代码与 audit 实现保持不变 | `.monkeycode/specs/gc-direct-local-lobby-indexed-compound-write-guard/`、`tools/test_audit_gc_refactor.py` |
| 2026-07-26 | direct target Local lobby indexed compound write guard：补充 audit 10d 回归，显式覆盖 `options.client_target->GBE_local_lobby.members[0].team += 1u` target indexed compound mutation，生产代码与 audit 实现保持不变 | `.monkeycode/specs/gc-direct-target-local-lobby-indexed-compound-write-guard/`、`tools/test_audit_gc_refactor.py` |
| 2026-07-26 | direct target Local lobby indexed postfix write guard：补充 audit 10d 回归，显式覆盖 `options.client_target->GBE_local_lobby.members[0].team++` target indexed postfix mutation，生产代码与 audit 实现保持不变 | `.monkeycode/specs/gc-direct-target-local-lobby-indexed-postfix-write-guard/`、`tools/test_audit_gc_refactor.py` |
| 2026-07-26 | direct Local lobby indexed postfix write guard：补充 audit 10d 回归，显式覆盖 `GBE_local_lobby.members[0].team++` indexed postfix mutation，生产代码与 audit 实现保持不变 | `.monkeycode/specs/gc-direct-local-lobby-indexed-postfix-write-guard/`、`tools/test_audit_gc_refactor.py` |
| 2026-07-26 | direct target Local lobby indexed prefix write guard：补充 audit 10d 回归，显式覆盖 `++options.client_target->GBE_local_lobby.members[0].team` target indexed prefix mutation，生产代码与 audit 实现保持不变 | `.monkeycode/specs/gc-direct-target-local-lobby-indexed-prefix-write-guard/`、`tools/test_audit_gc_refactor.py` |
| 2026-07-26 | direct Local lobby indexed prefix write guard：audit 10d 覆盖 `++GBE_local_lobby.members[0].team` / indexed field 前缀 mutation，生产代码保持不变 | `.monkeycode/specs/gc-direct-local-lobby-indexed-prefix-write-guard/`、`tools/_audit_gc_refactor.py` |
...
| 2026-07-26 | direct Local lobby decltype auto alias guard：audit 10d 覆盖 `decltype(auto) lobby = (GBE_local_lobby)` parenthesized reference alias 形式，生产代码保持不变 | `.monkeycode/specs/gc-direct-local-lobby-decltype-auto-alias-guard/`、`tools/_audit_gc_refactor.py` |
| 2026-07-26 | direct Local lobby forwarding alias guard：audit 10d mutable alias matcher 覆盖 `auto&& lobby = GBE_local_lobby` forwarding reference 形式，生产代码保持不变 | `.monkeycode/specs/gc-direct-local-lobby-forwarding-alias-guard/`、`tools/_audit_gc_refactor.py` |
...
| 2026-07-26 | client lobby restore snapshot apply helper：custom game lifecycle cross-GC mirror 的 client Local 快照恢复经 `apply_client_lobby_restore_snapshot(...)`，client target guard、active/lobby_id guard、launch peripheral reset 与 last launch state clear 顺序保持不变 | `.monkeycode/specs/gc-local-client-lobby-restore-snapshot-apply/`、`LOCAL_LOBBY_USAGE.md` |
