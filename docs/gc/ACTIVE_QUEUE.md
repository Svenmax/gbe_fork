...
- CompositionRoot 进生产
- 为美观再横向大拆文件
- 继续横向给所有 full adopt 字段加 generation marker
...
| 日期 | 摘要 | 链接 |
|------|------|------|
| 2026-07-28 | GC dll domain boundary cleanup：固定 Coordinator/Lobby/MatchRuntime/Inventory/ChatBroadcast/Reconnect/PayloadWire/TemplateReplay 边界，明确停止继续碎拆文件和横向扩 marker | `GC_DLL_DOMAIN_BOUNDARY_CLEANUP.md` |
...
| 2026-07-28 | source-aware local lobby restore guards：runtime、options/custom_game、cache、owner、members、details、chat/broadcast 等常见 shared restore/adopt 覆盖点增加 source-aware guard | `dll/gbe_dota_lobby_state.cpp`、`LOCAL_LOBBY_USAGE.md` |
| 2026-07-28 | production GC TU consolidation：合并低价值拆分文件，production GC translation units 从 57 收敛到 52 | `dll/gbe_dota_*` |
| 2026-07-26 | direct Local lobby write guard 扩展：audit 10d 覆盖 direct object/field/indexed/alias/mutating-method 写入旁路，Local 写入须经命名 state helper | `tools/_audit_gc_refactor.py`、`tools/test_audit_gc_refactor.py` |
