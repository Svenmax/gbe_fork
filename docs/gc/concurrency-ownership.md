...
| Serialized connection state | One `GBE_DotaSerializedConnectionState` per `Steam_Networking_Sockets_Serialized` instance | Instance call path currently serialized by caller-held `global_mutex` in `PostConnectionStateMsg()` | `GBE_ExecuteDotaReconnectPostConnectionState()` and state methods called from the owning serialized sockets instance | State never crosses instances and exposes no process-global mutable backing. Concurrent entry requires explicit instance serialization before P11.3 can relax reliance on `global_mutex`. |
...
| Callback queue | Client/server `SteamCallBacks` and `SteamCallResults` objects owned by `Steam_Client` | Queue mutation and draining occur while `Steam_Client::RunCallbacks()` holds `global_mutex` | `addCBResult()` copies payload and guard context; `runCallResults()` evaluates guards and delivers ready callbacks | Queue containers remain protected while selecting and copying work. User callback execution occurs after releasing `global_mutex`, followed by reacquisition before queue traversal continues. |
...
- `Steam_Networking_Sockets_Serialized::PostConnectionStateMsg()` enters through `global_mutex` and owns all mutation of its `dota_connection_state` during that call.
...
- `SteamCallResults::runCallResults()` releases `global_mutex` around user callback invocation and reacquires it before continuing internal queue traversal.
...
6. Treat `global_mutex` as the current outer process synchronization domain; P11.2 will define and enforce a narrower lock order for store, callback, and network effects.

## Known Follow-Up Boundaries

- Recent reconnect context relies on caller-held `global_mutex`; P11.2 must make that requirement mechanically clear or give it a dedicated synchronization owner.
- `PostConnectionStateMsg()` currently invokes direct networking and callback queue adapters while holding `global_mutex`; P11.2 moves external effects beyond the process lock boundary.
...
- `SteamCallResults::runCallResults()` contains manual unlock/relock behavior around callbacks; P11.2 defines the lock order and removes recursive-lock ambiguity from this path.
