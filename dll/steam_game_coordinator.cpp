...
static void GBE_BuildDotaLobbyMemberObject2004(const GBE_DotaLobbyMemberState &member, uint32 lobby_state, uint32 lobby_game_state, std::string &member_state)
{
    ...
    // Leaver status (field 16):
    // Pre-game (lobby UI/setup): Real GC always writes field 16 = 1 for all members,
    //   which Dota uses to determine member presence/color in the lobby chat panel.
    // In-game (state=RUN, game_state>=1): field 16 is only written for disconnected
    //   members (value=1=DISCONNECTED); connected members omit it entirely.
    ...
    if (effective_leaver != 0u) {
        ...
        // Pre-game: write field 16 = 1 to match real GC behavior
    ...
    std::string &message)
{
    ...
    auto append_channel_member = [&](uint64 member_steam_id, const std::string &member_name) {
        ...
        GBE_AppendProtoVarIntField(member, 3u, 0u);                 // channel_user_id (0, matches real GC)
    ...
    GBE_AppendProtoVarIntField(body, 9u, 0u);                    // channel_user_id (0, matches real GC)
...
static bool GBE_BuildDotaReadyUpStatusPayload(
    bool has_request_job,
    uint64 request_job_id,
    uint64 lobby_id,
    uint32 state,
...
static bool GBE_IsDotaCustomReadyUpLaunchState(const Steam_Game_Coordinator::GBE_LocalLobby &lobby)
{
    return lobby.active &&
        lobby.lobby_id != 0ull &&
        lobby.state == 4u &&
        lobby.game_state == 0u &&
        lobby.launch_phase == GBE_kDotaLaunchPhaseRequested &&
        (GBE_HasDotaCustomGameDetails(lobby.custom_game) || lobby.match_id != 0ull || lobby.server_id != 0ull || !lobby.connect.empty());
}
...
bool Steam_Game_Coordinator::GBE_HandleDotaDirectPostLoginRequest(uint32 unMsgType, const void *pubData, uint32 cubData)
{
    ...
    if (request_emsg == 7070u) {
        ...
        const bool shared_readyup_context =
            GBE_shared_dota_lobby_state.valid &&
            GBE_IsDotaCustomReadyUpLaunchState(GBE_shared_dota_lobby_state) &&
            (!GBE_local_lobby.active || GBE_local_lobby.lobby_id == 0ull || GBE_local_lobby.lobby_id == GBE_shared_dota_lobby_state.lobby_id);
        const bool custom_readyup_context = GBE_IsDotaCustomReadyUpLaunchState(GBE_local_lobby) || shared_readyup_context;
        if (custom_readyup_context) {
            if (shared_readyup_context && !GBE_IsDotaCustomReadyUpLaunchState(GBE_local_lobby)) {
                GBE_local_lobby = GBE_shared_dota_lobby_state;
                GBE_GC_DebugLog(
                    "GC_DOTA_LOBBY",
                    "[LOBBY] Adopted shared custom READYUP before direct 7070 lobby_id=%llu state=%u game_state=%u launch_phase=%s custom_id=%llu",
                    static_cast<unsigned long long>(GBE_local_lobby.lobby_id),
                    GBE_local_lobby.state,
                    GBE_local_lobby.game_state,
                    GBE_DescribeDotaLaunchPhase(GBE_local_lobby.launch_phase),
            ...
            if (GBE_local_lobby.state == 4u && GBE_local_lobby.game_state == 0u && GBE_local_lobby.launch_phase == GBE_kDotaLaunchPhaseRequested) {
                GBE_SendDotaCustomGameServerSetupAfterReady(false, nullptr, has_source_job, source_job, ready_state != 0u ? ready_state : 1u);
    ...
    if (request_emsg == 8052u) {
        ...
        if (GBE_local_lobby.active && GBE_local_lobby.lobby_id != 0 && GBE_HasDotaCustomGameDetails(GBE_local_lobby.custom_game)) {
            ...
            GBE_ExtractProtoFieldUint64(body, body_size, GBE_FindProtoField(body, body_size, 2u), custom_game_id);
            GBE_ExtractProtoFieldUint64(body, body_size, GBE_FindProtoField(body, body_size, 4u), start_time);
    ...
    switch (request_emsg) {
        ...
        case 7091: {
            ...
            // Second response: READY with server info
            {
                ...
                // field 2: source_tv_public_addr (uint32 IP)
                if (source_tv_addr != 0)
                    GBE_AppendProtoVarIntField(ready_body, 2u, source_tv_addr);
                // field 3: source_tv_private_addr (same as public for LAN)
                if (source_tv_addr != 0)
                    GBE_AppendProtoVarIntField(ready_body, 3u, source_tv_addr);
                // field 4: source_tv_port
...
bool Steam_Game_Coordinator::GBE_SendDotaPracticeLobbyLaunchMessage(uint32 inner_emsg, const std::string &inner_message, bool wrapped, const std::string *outer_session_field_raw, const char *reason, bool apply_lobby_state, uint32 lobby_state, uint32 lobby_game_state)
{
    if (wrapped) {
        if (!outer_session_field_raw) {
            GBE_GC_DebugLog("GC_DOTA_LOBBY", "[LOBBY] Missing wrapped session context for launch message emsg=%u reason=%s", inner_emsg, reason ? reason : "unknown");
            return false;
        }
        ...
        if (!GBE_BuildWrappedDotaReplayMessage(inner_message, *outer_session_field_raw, settings->get_local_steam_id().ConvertToUint64(), wrapped_message)) {
            GBE_GC_DebugLog("GC_DOTA_LOBBY", "[LOBBY] Failed wrapping launch message emsg=%u reason=%s", inner_emsg, reason ? reason : "unknown");
            return false;
        }
...
bool Steam_Game_Coordinator::GBE_SendDotaCustomGameLaunchSetupFlow(bool wrapped, const std::string *outer_session_field_raw, bool has_request_job, uint64 request_job_id)
{
    ...
    GBE_GC_DebugLog(
        "GC_DOTA_LOBBY",
        "[LOBBY] Sent custom game READYUP after 7041 path=%s has_request_job=%d request_job=%llu LobbyID=%llu match_id=%llu server_id=%llu custom_id=%llu custom_map=%s",
        wrapped ? "wrapped" : "direct",
        has_request_job ? 1 : 0,
        static_cast<unsigned long long>(request_job_id),
        static_cast<unsigned long long>(GBE_local_lobby.lobby_id),
...
bool Steam_Game_Coordinator::GBE_SendDotaCustomGameServerSetupAfterReady(bool wrapped, const std::string *outer_session_field_raw, bool has_request_job, uint64 request_job_id, uint32 local_ready_state)
{
    std::string response_7170;
    if (!GBE_BuildDotaReadyUpStatusPayload(has_request_job, request_job_id, GBE_local_lobby.lobby_id, 0u, local_ready_state != 0u ? local_ready_state : 1u, response_7170)) {
        GBE_GC_DebugLog("GC_DOTA_LOBBY", "[LOBBY] Failed building custom game 7170 after 7070 LobbyID=%llu", static_cast<unsigned long long>(GBE_local_lobby.lobby_id));
        return false;
    }

    if (!GBE_SendDotaPracticeLobbyLaunchMessage(7170u, response_7170, wrapped, outer_session_field_raw, "7070_custom_game_7170"))
        return false;

    GBE_local_lobby.state = 1u;
    GBE_local_lobby.game_state = 0u;
    GBE_PublishSharedDotaLobbyState("7070_custom_game_serversetup");
    ...
    if (!GBE_BuildAuthoritativeDotaPracticeLobbyDetailsUpdate(GBE_local_lobby, GBE_local_lobby.owner_name, serversetup_26, true)) {
        GBE_GC_DebugLog("GC_DOTA_LOBBY", "[LOBBY] Failed building custom game SERVERSETUP 26 after 7070 LobbyID=%llu", static_cast<unsigned long long>(GBE_local_lobby.lobby_id));
        return false;
    }

    if (!GBE_SendDotaPracticeLobbyLaunchMessage(GBE_kDotaPracticeLobbyDetailsUpdate, serversetup_26, wrapped, outer_session_field_raw, "7070_custom_game_serversetup", true, 1u, 0u))
        return false;

    GBE_MarkDotaLaunchPhase(GBE_kDotaLaunchPhaseSetupSynced, "7070_custom_game_serversetup_synced");
    GBE_GC_DebugLog(
        "GC_DOTA_LOBBY",
        "[LOBBY] Sent custom game SERVERSETUP after 7070 path=%s has_request_job=%d request_job=%llu local_ready_state=%u LobbyID=%llu match_id=%llu server_id=%llu custom_id=%llu custom_map=%s",
        wrapped ? "wrapped" : "direct",
        has_request_job ? 1 : 0,
        static_cast<unsigned long long>(request_job_id),
        local_ready_state,
...
bool Steam_Game_Coordinator::GBE_HandleDotaWrappedPostLoginRequest(const void *pubData, uint32 cubData)
{
    ...
    if (context.inner_emsg != GBE_kDotaJoinChatChannel)
        ...
        if (context.inner_emsg != 7091u) // WatchGame
            return false;
    ...
    if (context.inner_emsg == 7070u) {
        ...
        if (custom_readyup_context) {
            if (shared_readyup_context && !GBE_IsDotaCustomReadyUpLaunchState(GBE_local_lobby)) {
                ...
                GBE_GC_DebugLog(
                    "GC_DOTA_LOBBY",
                    "[LOBBY] Adopted shared custom READYUP before wrapped 7070 lobby_id=%llu state=%u game_state=%u launch_phase=%s custom_id=%llu",
                    static_cast<unsigned long long>(GBE_local_lobby.lobby_id),
                    GBE_local_lobby.state,
                    GBE_local_lobby.game_state,
                    GBE_DescribeDotaLaunchPhase(GBE_local_lobby.launch_phase),
            ...
            if (GBE_local_lobby.state == 4u && GBE_local_lobby.game_state == 0u && GBE_local_lobby.launch_phase == GBE_kDotaLaunchPhaseRequested) {
                GBE_SendDotaCustomGameServerSetupAfterReady(true, &context.outer_session_field_raw, context.has_request_job, context.request_job_id, ready_state != 0u ? ready_state : 1u);
            } else if (GBE_local_lobby.state == 1u && GBE_local_lobby.game_state == 0u) {
    ...
    if (context.inner_emsg == GBE_kDotaFindTopSourceTVGames) {
        ...
        // Wrap as CMsgGCClient for the ClientFromGC channel
        std::string wrapped_response;
        if (!GBE_BuildWrappedDotaReplayMessage(response_message, context.outer_session_field_raw, settings->get_local_steam_id().ConvertToUint64(), wrapped_response)) {
            GBE_GC_DebugLog("GC_DOTA_LOBBY", "[WATCH] Failed wrapping 8010 response");
            return true;
        }
    ...
    if (context.inner_emsg == 7091u) {
        ...
        std::string pending_body;
        GBE_AppendProtoVarIntField(pending_body, 1u, 0u);
        std::string pending_inner;
        GBE_BuildDotaJobReplyOrZeroHeaderPayload(7092u, context.has_request_job, context.request_job_id, pending_body, pending_inner);
        std::string wrapped_pending;
        ...
        std::string wrapped_ready;
        if (GBE_BuildWrappedDotaReplayMessage(ready_inner, context.outer_session_field_raw, settings->get_local_steam_id().ConvertToUint64(), wrapped_ready))
