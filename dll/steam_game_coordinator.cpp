...
static bool GBE_dota_launch_pending_prelaunch_game_state_7034 = false;
static uint32 GBE_dota_launch_peripheral_stage_mask = 0;

enum : uint32 {
    GBE_kDotaLaunchPeripheralStageInitPersona = 1u << 0,
    GBE_kDotaLaunchPeripheralStageServersAvailable = 1u << 1,
    GBE_kDotaLaunchPeripheralStageAuthListAck1 = 1u << 2,
    GBE_kDotaLaunchPeripheralStageGameConnectTokens1 = 1u << 3,
    GBE_kDotaLaunchPeripheralStageSetupPersona = 1u << 4,
    GBE_kDotaLaunchPeripheralStageAuthListAck2 = 1u << 5,
    GBE_kDotaLaunchPeripheralStageGameConnectTokens2 = 1u << 6,
    GBE_kDotaLaunchPeripheralStageRunPersona = 1u << 7,
    GBE_kDotaLaunchPeripheralStageTicketAuthComplete = 1u << 8,
    GBE_kDotaLaunchPeripheralStagePrivateLobbyPersona = 1u << 9,
    GBE_kDotaLaunchPeripheralStageCacheSubscribedPrelude = 1u << 10,
    GBE_kDotaLaunchPeripheralStageCacheSubscribedRuntimePrelude = 1u << 11,
    GBE_kDotaLaunchPeripheralStageServersAvailablePrelude = 1u << 12,
    GBE_kDotaLaunchPeripheralStageAuthListAckPrelude = 1u << 13,
    GBE_kDotaLaunchPeripheralStageServersAvailablePost8744 = 1u << 14,
    GBE_kDotaLaunchPeripheralStageAuthListAckLateSteamChain = 1u << 15,
...
static void GBE_BuildDotaServerStaticLobbyObject2016(uint32 account_id, uint64 steam_id, std::string &object_2016)
{
    ...
    GBE_AppendProtoFixed64Field(member_bytes, 1u, steam_id);
...
void Steam_Game_Coordinator::GBE_ApplyQueuedLobbyState(const GC_Message &message)
{
    ...
    // In the hero-selection window, queue another Steam-side runtime 26 before
    // the late persona messages arrive. Do not reuse Dota-process donor packets
    // here because their 24/26 timing diverges from the Steam-side session.
    if (GBE_local_lobby.active &&
        ...
        if (GBE_BuildCurrentDotaPracticeLobbyDetailsUpdate(GBE_local_lobby, GBE_GetDotaLobbyOwnerName(), current_26)) {
            ...
            GBE_GC_DebugLog(
                "GC_DOTA_SYNC",
                "queued hero-selection current 26 before persona lobby_id=%llu match_id=%llu server_id=%llu size=%zu source=runtime",
...
bool Steam_Game_Coordinator::GBE_TryQueueDotaRuntimeLobbyDetailsUpdate(const char *note, uint32 trigger_emsg, uint64 source_job, uint32 next_state, uint32 next_game_state)
{
    ...
    if (!GBE_BuildCurrentDotaPracticeLobbyDetailsUpdate(next_lobby, GBE_local_lobby.owner_name, response_message))
        return false;
...
void Steam_Game_Coordinator::GBE_PublishSharedDotaLobbyState(const char *reason)
{
    ...
    GBE_GC_DebugLog(
        "GC_DOTA_SYNC",
        "published shared lobby this=%p shared_lobby=%p reason=%s active=%u lobby_id=%llu generic_lobby_id=%llu match_id=%llu owner_steam_id=%llu owner_account_id=%u state=%u game_state=%u team=%u slot=%u connect=%s",
        static_cast<void *>(this),
        static_cast<void *>(&GBE_shared_dota_lobby_state),
        reason ? reason : "unknown",
...
void Steam_Game_Coordinator::GBE_MaybeReplayCurrentDotaPrivateLobbySnapshot(const char *reason)
{
    ...
    if (!GBE_BuildCurrentDotaPracticeLobbyCacheSubscribedTemplateReplay(lobby, GBE_GetDotaLobbyOwnerName(), response_24)) {
        GBE_GC_DebugLog(
            "GC_DOTA_SYNC",
    ...
    if (!GBE_BuildCurrentDotaPracticeLobbyDetailsUpdate(lobby, GBE_GetDotaLobbyOwnerName(), response_26)) {
        GBE_GC_DebugLog(
            "GC_DOTA_SYNC",
    ...
    if (previous_server_id == 0) {
        ...
        if (steam_id != 0 && account_id != 0) {
            ...
            if (GBE_BuildDotaPracticeLobbyLaunchCacheSubscribedTemplateReplay(
                ...
                GBE_RecordDotaLobbyCacheSubscriptionState(runtime_cache_message, reason ? reason : "server_id_sync_official_template");
        ...
        GBE_QueueDotaPracticeLobbyLaunchPeripheralOnce(
            GBE_kDotaLaunchPeripheralStageServersAvailable,
            GBE_kSteamServersAvailable,
            GBE_kDotaPracticeLobbyLaunchServersAvailableHex,
            false,
            "launch servers available after server_id sync"
        );
        GBE_QueueDotaPracticeLobbyLaunchPeripheralOnce(
            GBE_kDotaLaunchPeripheralStageAuthListAck1,
            GBE_kSteamAuthListAck,
            GBE_kDotaPracticeLobbyLaunchAuthListAckStage1Hex,
            false,
            "launch auth list ack stage1 after server_id sync"
        );
        GBE_QueueDotaPracticeLobbyLaunchPeripheralOnce(
            GBE_kDotaLaunchPeripheralStageGameConnectTokens1,
            GBE_kSteamGameConnectTokens,
            GBE_kDotaPracticeLobbyLaunchGameConnectTokensStage1Hex,
            false,
            "launch game connect tokens stage1 after server_id sync"
        );
        GBE_QueueDotaPracticeLobbyLaunchPeripheralOnce(
            GBE_kDotaLaunchPeripheralStageSetupPersona,
            GBE_kSteamPersonaState,
            GBE_kDotaPracticeLobbyLaunchPersonaStateSetupHex,
            false,
...
bool Steam_Game_Coordinator::GBE_HandleDotaDirectPostLoginRequest(uint32 unMsgType, const void *pubData, uint32 cubData)
{
    ...
    if (request_emsg == 7034) {
        auto queue_official_26 = [&](const char *template_hex, uint32 next_state, uint32 next_game_state, bool rewrite_2015, uint32 extra_startup_account_id, const char *note) -> bool {
            ...
            push_incoming_now(GBE_kDotaPracticeLobbyDetailsUpdate | GBE_kProtoMask, response_message, true, next_state, next_game_state);
            GBE_GC_DebugLog(
                "GC_DOTA_DIRECT",
        ...
        if (GBE_local_lobby.active && GBE_local_lobby.lobby_id != 0 && GBE_local_lobby.match_id != 0 && GBE_local_lobby.server_id != 0) {
            ...
            if (GBE_local_lobby.state == 1u && GBE_local_lobby.game_state == 0u) {
                if (has_full_launch_context && is_game_state_7034 && !GBE_dota_launch_seen_4511)
                    GBE_dota_launch_pending_prelaunch_game_state_7034 = true;
                const std::string request_summary = GBE_FormatDota7034Summary(body, body_size);
                if (can_promote_prelaunch_7034) {
                    GBE_dota_launch_pending_prelaunch_game_state_7034 = false;
                    GBE_GC_DebugLog(
                        "GC_DOTA_DIRECT",
                        "consumed req=%u source_job=%llu note=prelaunch 7034 has full launch context plus matched 4511 and GAME_STATE reason and may promote to runtime wait_for_players active=%u lobby_id=%llu state=%u game_state=%u match_id=%llu server_id=%llu launch_seen_4511=%u summary=%s",
                        request_emsg,
                        static_cast<unsigned long long>(source_job),
                        GBE_local_lobby.active ? 1u : 0u,
                        static_cast<unsigned long long>(GBE_local_lobby.lobby_id),
                    ...
                    GBE_GC_DebugLog(
                        "GC_DOTA_DIRECT",
                        "consumed req=%u source_job=%llu note=prelaunch 7034 waits for matched 4511 plus GAME_STATE reason before runtime wait_for_players progression active=%u lobby_id=%llu state=%u game_state=%u match_id=%llu server_id=%llu launch_seen_4511=%u full_launch_context=%u is_game_state_reason=%u summary=%s",
                        request_emsg,
                        static_cast<unsigned long long>(source_job),
                        GBE_local_lobby.active ? 1u : 0u,
                        static_cast<unsigned long long>(GBE_local_lobby.lobby_id),
            ...
            if (GBE_local_lobby.state == 2u && GBE_local_lobby.game_state == 3u) {
                if (queue_official_26(GBE_kDotaOfficial032PracticeLobby26Hex, 2u, 10u, false, 0u, "official packet 032 after 7034")) {
                    ...
                    const uint32 account_id = GBE_GetDotaLobbyOwnerAccountId();
                    if (steam_id != 0 && account_id != 0) {
                        if (!(GBE_dota_launch_peripheral_stage_mask & GBE_kDotaLaunchPeripheralStageCacheSubscribedPrelude)) {
                            ...
                            if (GBE_BuildDotaPracticeLobbyLaunchCacheSubscribedPreludeTemplateReplay(
                                ...
                                GBE_dota_launch_peripheral_stage_mask |= GBE_kDotaLaunchPeripheralStageCacheSubscribedPrelude;
                                GBE_RecordDotaLobbyCacheSubscriptionState(prelude_cache_message, "official_032_prelude");
                                push_incoming_now(GBE_kDotaCacheSubscribed | GBE_kProtoMask, prelude_cache_message);
                                GBE_GC_DebugLog(
                                    "GC_DOTA_SYNC",
                                    "queued launch prelude CacheSubscribed after official 032 lobby_id=%llu size=%zu",
                                    static_cast<unsigned long long>(GBE_local_lobby.lobby_id),
                                ...
                                GBE_GC_DebugLog(
                                    "GC_DOTA_SYNC",
                                    "failed building launch prelude CacheSubscribed after official 032 lobby_id=%llu owner_steam_id=%llu owner_account_id=%u",
                                    static_cast<unsigned long long>(GBE_local_lobby.lobby_id),
                                    static_cast<unsigned long long>(steam_id),
                                    account_id
                                );
                        ...
                        if (!(GBE_dota_launch_peripheral_stage_mask & GBE_kDotaLaunchPeripheralStageCacheSubscribedRuntimePrelude)) {
                            std::string large_prelude_cache_message;
                            if (GBE_BuildDotaPracticeLobbyLaunchCacheSubscribedLargePreludeTemplateReplay(
                                    account_id,
                                    steam_id,
                                    GBE_local_lobby.lobby_id,
                                    large_prelude_cache_message)) {
                                GBE_dota_launch_peripheral_stage_mask |= GBE_kDotaLaunchPeripheralStageCacheSubscribedRuntimePrelude;
                                GBE_RecordDotaLobbyCacheSubscriptionState(large_prelude_cache_message, "official_032_large_prelude");
                                push_incoming_now(GBE_kDotaCacheSubscribed | GBE_kProtoMask, large_prelude_cache_message);
                                GBE_GC_DebugLog(
                                    "GC_DOTA_SYNC",
                                    "queued launch large prelude CacheSubscribed after official 032 lobby_id=%llu size=%zu",
                                    static_cast<unsigned long long>(GBE_local_lobby.lobby_id),
                                    large_prelude_cache_message.size()
                                );
                            } else {
                                GBE_GC_DebugLog(
                                    "GC_DOTA_SYNC",
                                    "failed building launch large prelude CacheSubscribed after official 032 lobby_id=%llu owner_steam_id=%llu owner_account_id=%u",
                                    static_cast<unsigned long long>(GBE_local_lobby.lobby_id),
                                    static_cast<unsigned long long>(steam_id),
                                    account_id
                                );
            ...
            if (!GBE_dota_launch_pending_8870 && !(GBE_local_lobby.state == 2u && GBE_local_lobby.game_state == 10u)) {
    ...
    if (request_emsg == 4506) {
        if (GBE_local_lobby.active &&
            ...
            if (!GBE_TryQueueDotaRuntimeLobbyDetailsUpdate("runtime packet prelude after 4506", request_emsg, source_job, 1u, 0u)) {
                ...
                if (GBE_BuildCurrentDotaPracticeLobbyDetailsUpdate(prelude_lobby, GBE_local_lobby.owner_name, prelude_message)) {
                    ...
                    GBE_GC_DebugLog(
                        "GC_DOTA_DIRECT",
                        "replying req=%u resp=%u source_job=%llu size=%zu note=runtime packet prelude after 4506 fallback apply_state=%u apply_game_state=%u",
                        request_emsg,
                        GBE_kDotaPracticeLobbyDetailsUpdate,
                        static_cast<unsigned long long>(source_job),
    ...
    if (request_emsg == 8870) {
        GBE_dota_launch_pending_8870 = GBE_local_lobby.active &&
            GBE_local_lobby.lobby_id != 0 &&
            GBE_local_lobby.match_id != 0 &&
            GBE_local_lobby.server_id != 0 &&
            GBE_local_lobby.state == 2u &&
        ...
        GBE_GC_DebugLog(
            "GC_DOTA_DIRECT",
            "consumed req=%u source_job=%llu note=official 8870 launch marker pending_followup=%u active=%u lobby_id=%llu state=%u game_state=%u",
            request_emsg,
            static_cast<unsigned long long>(source_job),
            GBE_dota_launch_pending_8870 ? 1u : 0u,
            GBE_local_lobby.active ? 1u : 0u,
            static_cast<unsigned long long>(GBE_local_lobby.lobby_id),
            GBE_local_lobby.state,
            GBE_local_lobby.game_state
    ...
    if (request_emsg == 4511) {
        ...
        if (matches_local_lobby &&
                GBE_dota_launch_pending_prelaunch_game_state_7034 &&
                GBE_local_lobby.active &&
                GBE_local_lobby.lobby_id != 0 &&
                GBE_local_lobby.match_id != 0 &&
                GBE_local_lobby.server_id != 0 &&
            ...
            GBE_GC_DebugLog(
                "GC_DOTA_DIRECT",
                "consumed req=%u source_job=%llu note=matched 4511 consumed deferred GAME_STATE 7034 and may promote to runtime wait_for_players active=%u lobby_id=%llu state=%u game_state=%u match_id=%llu server_id=%llu",
                request_emsg,
                static_cast<unsigned long long>(source_job),
                GBE_local_lobby.active ? 1u : 0u,
                static_cast<unsigned long long>(GBE_local_lobby.lobby_id),
            ...
            if (GBE_TryQueueDotaPrelaunch021("runtime wait_for_players after deferred 7034/4511", request_emsg, source_job))
                return true;
        }

        GBE_GC_DebugLog(
            "GC_DOTA_DIRECT",
            "consumed req=%u source_job=%llu note=lan server available notification lobby_id=%llu local_lobby_id=%llu matches_local=%u launch_seen_4511=%u deferred_game_state_7034=%u",
            request_emsg,
            static_cast<unsigned long long>(source_job),
            static_cast<unsigned long long>(lobby_id),
            static_cast<unsigned long long>(GBE_local_lobby.lobby_id),
            ...
            GBE_dota_launch_seen_4511 ? 1u : 0u,
            GBE_dota_launch_pending_prelaunch_game_state_7034 ? 1u : 0u
        );
        return true;
    }
...
bool Steam_Game_Coordinator::GBE_HandleDotaPracticeLobbyCreateRequest(const std::string &request_body, uint64 request_job_id, bool wrapped, const std::string *outer_session_field_raw)
{
    ...
    Steam_Client *steam_client = get_steam_client();
    if (steam_client && steam_client->steam_matchmaking) {
        CSteamID generic_lobby_id = steam_client->steam_matchmaking->CreateLobbyImmediate(k_ELobbyTypeInvisible, 1);
        if (generic_lobby_id.IsLobby())
...
bool Steam_Game_Coordinator::GBE_HandleDotaPracticeLobbyLeaveRequest(bool wrapped, const std::string *outer_session_field_raw)
{
    ...
    const bool preserve_lobby_for_launch =
        ...
        GBE_local_lobby.server_id == 0 &&
        GBE_local_lobby.game_start_time == 0;
    const bool was_private_lobby_launch_state =
        GBE_local_lobby.state == 2u &&
        GBE_local_lobby.game_state >= 2u;
...
bool Steam_Game_Coordinator::GBE_HandleDotaPracticeLobbyLaunchRequest(bool wrapped, const std::string *outer_session_field_raw)
{
    ...
    GBE_QueueDotaPracticeLobbyLaunchPeripheralOnce(
        GBE_kDotaLaunchPeripheralStageInitPersona,
        GBE_kSteamPersonaState,
        GBE_kDotaPracticeLobbyLaunchPersonaStateInitHex,
        false,
...
bool Steam_Game_Coordinator::GBE_ShouldTrackDotaPracticeLobbyLateSteamChain() const
{
    return
        ...
        GBE_local_lobby.state == 2u &&
        GBE_local_lobby.game_state >= 2u &&
        (GBE_dota_launch_peripheral_stage_mask & GBE_kDotaLaunchPeripheralStageServersAvailablePost8744);
}
...
bool Steam_Game_Coordinator::GBE_FinalizeDotaPracticeLobbyLateSteamChain(const char *reason)
{
    ...
    if (!GBE_QueueDotaPracticeLobbyLaunchPeripheralOnce(
            GBE_kDotaLaunchPeripheralStageAuthListAckLateSteamChain,
            GBE_kSteamAuthListAck,
            GBE_kDotaPracticeLobbyLaunchAuthListAckStage2Hex,
            false,
...
{
    if (GBE_dota_launch_peripheral_stage_mask & stage_bit)
        return true;

    const uint64 steam_id = GBE_GetDotaLobbyOwnerSteamId();
    if (!GBE_local_lobby.active || GBE_local_lobby.lobby_id == 0 || steam_id == 0) {
        GBE_GC_DebugLog(
            "GC_DOTA_SYNC",
            "skipping launch peripheral msg=%u note=%s active=%u lobby_id=%llu owner_steam_id=%llu",
            emsg,
            note ? note : "unknown",
            GBE_local_lobby.active ? 1u : 0u,
            static_cast<unsigned long long>(GBE_local_lobby.lobby_id),
    ...
    if (!GBE_BuildDotaPracticeLobbyLaunchPeripheralMessage(
        ...
        GBE_GC_DebugLog(
            "GC_DOTA_SYNC",
            "failed building launch peripheral msg=%u note=%s lobby_id=%llu server_id=%llu",
            emsg,
            note ? note : "unknown",
            static_cast<unsigned long long>(GBE_local_lobby.lobby_id),
            static_cast<unsigned long long>(GBE_local_lobby.server_id)
    ...
    GBE_dota_launch_peripheral_stage_mask |= stage_bit;
    push_incoming_now(emsg | GBE_kProtoMask, message);
    GBE_GC_DebugLog(
        "GC_DOTA_SYNC",
        "queued launch peripheral msg=%u note=%s lobby_id=%llu server_id=%llu size=%zu",
        emsg,
        note ? note : "unknown",
        static_cast<unsigned long long>(GBE_local_lobby.lobby_id),
        static_cast<unsigned long long>(GBE_local_lobby.server_id),
