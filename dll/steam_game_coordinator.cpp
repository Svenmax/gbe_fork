...
static bool GBE_PatchDotaTemplateIdentifiers(std::string &message, uint32 account_id, uint64 steam_id, bool replace_account, bool replace_steam_id)
{
    ...
    if (replace_steam_id) {
        ...
        if (!GBE_EncodeVarUint64WithExpectedSize(steam_id, GBE_kOldDotaSteamIdVarint.size(), encoded_steam_id))
            return false;
        if (!GBE_FindAndReplaceBytes(message, GBE_VectorFromBytes(GBE_kOldDotaSteamIdVarint.data(), GBE_kOldDotaSteamIdVarint.size()), encoded_steam_id))
            return false;
    }
    ...
    std::string &message)
{
...

    {
        ...
        GBE_AppendProtoVarIntField(lobby_details, 13, 0u);
        GBE_AppendProtoVarIntField(lobby_details, 14, 1u);
...

    {
        ...
        GBE_AppendProtoBytesField(name_entry, 1, std::string(GBE_kDotaLobbyPlayerName));
