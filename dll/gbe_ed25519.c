/* gbe_ed25519.c -- Minimal Ed25519 signing wrapper for Goldberg Emulator.
 *
 * Public domain.  This file is derived from TweetNaCl by Daniel J. Bernstein,
 * Tanja Lange, and Peter Schwabe.  Only the SHA-512, Ed25519 scalar/group
 * arithmetic, and detached signing path are kept.
 */
                     ...
                     const uint8_t *private_key)
{
    ...
    if (!signature || (!message && message_len != 0) || !private_key)
        return -1;
    if (message_len > (size_t)(~(u64)0) - 64)
        return -1;

    n = (u64)message_len;
    sm = (u8 *)malloc((size_t)n + 64);
    if (!sm)
        return -1;

    crypto_hash(d, private_key, 32);
    d[0] &= 248;
    d[31] &= 127;
    d[31] |= 64;
